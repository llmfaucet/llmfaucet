...
admin provider routes. The Oracle probe is optional locally; use
`apps/probe/.env.example` for its Cloudflare D1/KV variables and run one cycle
with `npm run probe -- --once`.
