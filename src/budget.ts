...
export async function consumeBudget(request: Request, env: Env): Promise<{ allowed: boolean; remaining: number; reset: number; registered: boolean }> {
  ...
  if (token && env.DB) {
    try {
      ...
      const sponsor = await env.DB.prepare('SELECT k.id, COALESCE(s.requests_per_day, 50) AS requests_per_day FROM api_keys k JOIN users u ON u.id = k.user_id LEFT JOIN sponsor_entitlements s ON s.github_id = u.github_id AND s.status = \'active\' WHERE k.key_hash = ? AND k.status = \'active\'').bind(keyHash).first<{ requests_per_day: number }>();
      if (sponsor) { registered = true; limit = sponsor.requests_per_day; }
    } catch { /* migration may not have been applied yet */ }
    if (!registered && (await env.DB.prepare('SELECT token FROM tokens WHERE token = ?').bind(token).first())) { registered = true; limit = REGISTERED_LIMIT; }
  }
  ...
  if (env.DB) {
    ...
    return { allowed: Boolean(row), remaining: row ? limit - used : 0, reset, registered };
  }
  const used = Number(await env.BUDGETS.get(key) ?? 0);
  if (used >= limit) return { allowed: false, remaining: 0, reset, registered };
  await env.BUDGETS.put(key, String(used + 1), { expirationTtl: 172800 });
  return { allowed: true, remaining: limit - used - 1, reset, registered };
}
