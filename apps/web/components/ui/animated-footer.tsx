...
function buildHandCells(
  ...
  // Keep both hands square so differently cropped source images occupy the
  // same canvas footprint.
  const rows = columns;
  ...
  const backgroundBrightness = cornerPoints.reduce((total, [col, row]) => {
    const offset = (row * columns + col) * 4;
    return total + (pixels[offset] * 0.299 + pixels[offset + 1] * 0.587 + pixels[offset + 2] * 0.114) / 255;
  }, 0) / cornerPoints.length;
  const foregroundIsDark = backgroundBrightness > 0.55;
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < columns; col++) {
      ...
      const brightness =
        ...
        255;
      // Dark source pixels are the transparent background. Bright pixels are
      // mapped to the visible end of the character ramp.
      const contrast = foregroundIsDark
        ? backgroundBrightness - brightness
        : brightness - backgroundBrightness;
      if (pixels[offset + 3] < 32 || contrast < 0.12) continue;
      const charIndex = Math.min(
        asciiChars.length - 1,
        Math.floor((foregroundIsDark ? 1 - brightness : brightness) * asciiChars.length),
      );
...
export function AnimatedFooter({
  ...
  const cc = charColor ?? (isDark ? "#803500" : "#7a3500");
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 flex items-center justify-between">
        <div
          ref={leftWrapRef}
          className="relative h-full w-[34%] opacity-60 will-change-transform sm:w-[28%]"
          style={{ transform: `translateX(-${offEdge}%)` }}
        >
          <canvas ref={leftCanvasRef} className="block size-full object-contain" />
        </div>
        <div
          ref={rightWrapRef}
          className="relative h-full w-[34%] opacity-60 will-change-transform sm:w-[28%]"
          style={{ transform: `translateX(${offEdge}%)` }}
        >
          <canvas ref={rightCanvasRef} className="block size-full object-contain" />
        </div>
      </div>
