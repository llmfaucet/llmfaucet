...
export function responsesRequest(body: Record<string, unknown>): NormalizedRequest {
  const messages = responsesInputToMessages(body.input); if (typeof body.instructions === 'string' && body.instructions.trim()) messages.unshift({ role: 'system', content: body.instructions });
  return normalize({ ...body, messages, max_tokens: body.max_tokens ?? body.max_output_tokens, tools: translateTools(body.tools, 'responses'), tool_choice: translateToolChoice(body.tool_choice, 'responses'), stream: body.stream }, 'chat');
}
