...
export class OpenAICompatibleAdapter implements ProviderAdapter {
  ...
  async getModels(): Promise<Model[]> {
    try {
      ...
      if (!Array.isArray(data.data) || data.data.length === 0 || !data.data.every((item) => item && typeof item.id === 'string' && item.id.length > 0)) throw new Error('model catalog response is not a valid non-empty snapshot');
